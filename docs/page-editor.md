...
## Connected product galleries

The editor recognizes the main carousel and thumbnail strip as one Product gallery in common Funnelish/Swiper, Shopify media-gallery and native gallery markup. Other sliders, including reviews, keep their content. Select the gallery to manage its slides at the top of Content, or add a Product gallery from Add content.

Choose a product and Connect product media to use its full media list. Only this gallery creates a page-specific variation. Product media edits are staged as a local variation until the explicit Save product media action updates the catalog; the page Save button saves the page's connection and appearance. The separate product action affects every gallery connected to that product, including published pages, and provides conflict-aware Undo product media update. Stale saves are refused. Reload product media explicitly picks up the latest catalog version.

Slides support upload/asset selection, regeneration with branding and custom suggestions, descriptions, reordering, making a slide first, removal and direct video files. Thumbnails and arrows update with the slides; one-slide and empty galleries hide unnecessary controls. Image fit, proportions, thumbnail size/spacing, arrows, thumbnails and looping are editable. Touch swipes and keyboard controls work in preview and on the storefront. Page-only edits, connections and appearance use page Undo/Redo. Imported shells and slide classes remain; supported galleries use Storemill's carousel controller. Restore imported gallery returns to the source markup.

Duplicating an entire asset remaps gallery connections to the copied products. Duplicating a funnel or gallery inside the same store retains shared product references; use Only this gallery for an independent variation. Reusing a page template retargets the gallery to its destination product or retains local slides if no product is selected. Unrecognized custom carousel markup can remain editable as source; gallery recognition does not automatically replace testimonial sliders or guess a product from an unrelated image.

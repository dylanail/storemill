...
Select an image, background image, video or video poster, then choose **Regenerate / edit** in its Content settings. Native blocks expose the same controls beside their image/video settings. **Upload / choose asset** opens replacement controls directly. Embedded YouTube/Vimeo players require an uploaded original video; replacement converts the player to a direct video element.

The dialog starts with **Similar variation**: it keeps a similar pose, setting and composition without replacing logos or words. Use **Edit suggestions** and **Keep unchanged** to guide the difference. Choose **Change words** to edit specific text, or **Replace branding** to reveal the desired brand and logo controls. **Choose logo asset** opens all image assets so uncategorized logos are available; **Upload logo** opens the file picker. Add up to three other reference images and describe their intended roles. **Keep unchanged** specifies details to retain. Edit goals include branding, custom instructions, object/text removal, background changes, lighting/clarity and restyling. Image output can retain its original dimensions or use square, landscape or portrait format. AI images are new interpretations: review text, logos and product details in the preview.
...
- **AI image edit** offers similar variations, text changes, branding replacement and custom instructions. Brand controls appear for branding and custom edits. Branding replacement edits existing names and logos on packaging, products or graphics. Images use the selected OpenAI or Google image provider. Videos use an edited reference frame plus Runway Aleph 2 to guide the change through the footage. Pick a reference time where the branding is visible.
...
For AI image replacement, configure `OPENAI_API_KEY` or `GEMINI_API_KEY` and restart. The default OpenAI model is **GPT Image 2.5 Sunburst**, `gpt-image-2.5-sunburst`. Configure `STOREMILL_IMAGE_MODEL` and `STOREMILL_GOOGLE_IMAGE_MODEL` (Google default `gemini-3-pro-image-preview`); the legacy `AMBORAS_*` equivalents remain fallbacks. Explicit model overrides are preserved and the selector shows the configured model. Deployments pinned to GPT Image 2 must update their environment variable to enable 2.5. AI video replacement also requires `RUNWAYML_API_SECRET`. Keys remain server-side. Provider charges apply when a preview is generated. A missing key or provider error is shown explicitly; no synthetic fallback is substituted for an AI result.

Reference documentation: [GPT Image 2.5 Sunburst](https://developers.openai.com/api/docs/models/gpt-image-2.5-sunburst), [OpenAI image editing](https://developers.openai.com/api/docs/guides/image-generation), [Runway input requirements](https://docs.dev.runwayml.com/assets/inputs/), [Runway Aleph 2 release and limits](https://docs.dev.runwayml.com/api-details/api_changelog/).

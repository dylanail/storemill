...
The admin header uses the full white wordmark at 114 × 30 px on larger screens (reduced from 128 × 34 px) and the icon on mobile. The GitHub repository is [dylanail/storemill](https://github.com/dylanail/storemill). The assistant launcher and header use the supplied icon. Login, registration, onboarding, and password recovery share the black wordmark. Admin/editor document titles, application metadata, email branding, assistant platform context, first-party plugin labels, and new backup filenames use storemill.

...
for(const width of [1440,820,390,320])await suite.test('generated pages, buttons and checkout at '+width+'px',async t=>{
 ...
 try{
  ...
  await t.test('product gallery and variants update, cart quantities and discount work, checkout fits',async()=>{
   ...
   if(width<=640){for(const name of ['line2','phone']){const field=await page.locator('#checkout-form [name='+name+']').boundingBox();assert.ok(field.width>=width-80,'Phone checkout '+name+' field uses the available row width');}}
   if(width<=640){const state=await page.locator('#checkout-form [name=state]').boundingBox();const postal=await page.locator('#checkout-form [name=postal]').boundingBox();assert.ok(state.width>=130&&postal.width>=130,'State and postal inputs stay usable side by side');}
...
}finally{await context.close();}});
{
 ...
 for(const width of [1440,820,390,320])await suite.test((process.env.STOREFRONT_CHECKOUT_FIXTURE?'actual funnel':'copied fixture')+' checkout form and summary at '+width+'px',async()=>{const {context,page}=await newPage(width);try{
  ...
  await page.goto(base+'/checkout');await page.waitForFunction(()=>window.__COPY_COMMERCE_READY);assert.equal(await page.locator('#checkout-form').count(),1);if(width<=640){const state=await page.locator('#checkout-form [name=state],[name=state][form=checkout-form]').boundingBox();const postal=await page.locator('#checkout-form [name=postal],[name=postal][form=checkout-form]').boundingBox();assert.ok(state.width>=130&&postal.width>=130,'State and postal inputs stay usable side by side');}
   assert.equal(await page.locator('#checkout-form [name=email],[name=email][form=checkout-form]').isVisible(),true);assert.equal(await page.locator('[name=cardNumber],[name=cardDate],[name=cardSecurityCode],[name=emailAddress]').count(),0);assert.equal(await page.locator('#pay').isEnabled(),true);await fits(page,'Actual copied checkout');
  for(const name of ['email','country','firstName','lastName','line1','line2','city','state','postal','phone']){const field=page.locator('#checkout-form [name='+name+'],[name='+name+'][form=checkout-form]');assert.equal(await field.evaluate(el=>el.form?.id),'checkout-form');await field.scrollIntoViewIfNeeded();const box=await field.boundingBox();assert.ok(box&&box.width>=100&&box.height>=44&&box.x>=0&&box.x+box.width<=width+1,'Source styles must not collapse, float or clip '+name+' at '+width+'px');}
  ...
  if(width>740){const summaryBox=await (await summary.count()?summary.locator('summary'):page.locator('[data-owned-checkout-summary]')).boundingBox(),emailBox=await page.locator('#checkout-form [name=email],[name=email][form=checkout-form]').boundingBox();assert.ok(summaryBox.y<=emailBox.y+80,'Copied tablet/desktop summary begins beside the form, not at the bottom of its column');}

...
}finally{await context.close();}});
{
 ...
 for(const width of [1440,820,390,320])await suite.test((process.env.STOREFRONT_CHECKOUT_FIXTURE?'actual funnel':'copied fixture')+' checkout form and summary at '+width+'px',async()=>{const {context,page}=await newPage(width);try{
  ...
  await page.goto(base+'/checkout');await page.waitForFunction(()=>window.__COPY_COMMERCE_READY);assert.equal(await page.locator('#checkout-form').count(),1);if(width<=640){const state=await page.locator('#checkout-form [name=state]').boundingBox();const postal=await page.locator('#checkout-form [name=postal]').boundingBox();assert.ok(state.width>=130&&postal.width>=130,'State and postal inputs stay usable side by side');}
   assert.equal(await page.locator('#checkout-form [name=email]').isVisible(),true);assert.equal(await page.locator('[name=cardNumber],[name=cardDate],[name=cardSecurityCode],[name=emailAddress]').count(),0);assert.equal(await page.locator('#pay').isEnabled(),true);await fits(page,'Actual copied checkout');

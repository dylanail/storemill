...
test('editing exact bundle prices in the backend updates copied cards and keeps comparison prices',async()=>{
  ...
  await page.locator('[data-discount-edit="'+bundle.id+'"]').click();
  const form=page.locator('[data-discount-form]');
  assert.equal(await form.locator('[name="tier.1.value"]').inputValue(),'93.42');
  assert.equal(await form.locator('[name="tier.1.compare"]').inputValue(),'219.80');
  await form.locator('[name="tier.1.value"]').fill('90.01');
  await form.getByRole('button',{name:'Save discount',exact:true}).click();
  ...
  assert.equal(await page.locator('[data-copy-bundle-quantity="2"] .kaching-bundles__bar-price').textContent(),'$90.01');
  await page.locator('[data-copy-bundle-quantity="2"]').click();await page.getByRole('button',{name:'Add to cart',exact:true}).click();
  await page.waitForFunction(()=>document.querySelector('[data-copy-line-total]')?.textContent==='$90.01');

...
export function healthCard(ctx: Ctx, run: boolean, selected?: 'draft' | 'live'): string {
  ...
  const report=auditStore(ctx.db,ctx.store,{environment:selected ?? (ctx.store.status === 'live' ? 'live' : 'draft')}),jobs=report.environment === 'draft' ? healthFixes(ctx.db,ctx.store.id) : []
  const draft = report.environment === 'draft'
  const switcher = `<nav aria-label="Audit environment" class="row"><a class="btn ${draft ? 'primary' : ''}" href="/admin/speed?environment=draft" ${draft ? 'aria-current="page"' : ''}>Draft repairs</a><a class="btn ${!draft ? 'primary' : ''}" href="/admin/speed?environment=live" ${!draft ? 'aria-current="page"' : ''}>Live-site audit</a></nav>`
  return `<div id="health"><div class="card"><div class="row" style="justify-content:space-between"><div><h2>Store Speed</h2><p class="muted">${report.pages.length} page URLs checked · ${draft ? 'saved pages and draft theme' : 'published pages and live theme'} · up to 3 products</p></div><span class="tag ${report.score>=90?'ok':report.score>=70?'warn':'bad'}">Health score ${report.score}</span><a class="btn" href="/admin/speed?environment=${report.environment}">Run again</a></div>${switcher}<p class="muted">${draft ? 'Review your saved draft before publishing.' : 'Rendered with the live theme visitors see. Open Draft repairs to prepare a repair.'}</p><p class="muted">This is a source audit, not a measured load time or Core Web Vitals score. Compressed sizes estimate HTML only; images, fonts and network timings are not included.</p>${draft ? '<p class="muted">AI checks its repair before applying it. Saved page changes affect published pages; theme repairs stay in draft until you publish. Every applied fix can be undone.</p>' : ''}</div>
  <div id="health-notice" role="status" aria-live="polite"></div>
  ${report.pages.map(page=>`<details class="card" ${page.issues.length?'open':''}><summary><strong>${e(page.title)}</strong> <span class="muted">${e(page.path)} · ${(page.gzipBytes/1024).toFixed(1)} KB gzip HTML · ${page.issues.length} finding${page.issues.length===1?'':'s'}</span></summary>${page.issues.length?page.issues.map(issue=>{const job=jobs.find(job=>job.path===page.path&&job.check_name===issue.check&&job.status==='running');return `<article style="border-top:1px solid var(--line);padding:14px 0"><span class="tag ${issue.severity==='error'?'bad':'warn'}">${e(issue.check)}</span><p>${e(issue.detail)}</p><p><strong>Suggested fix:</strong> ${e(suggestedFix(issue))}</p>${draft ? `<button type="button" class="btn primary" data-health-fix data-path="${e(page.path)}" data-check="${e(issue.check)}" ${job?'disabled':''}>${job?'AI is fixing…':'Fix with AI'}</button>` : '<a class="btn" href="/admin/speed?environment=draft">Review in draft</a>'}</article>`}).join(''):'<p class="muted">No issues found by these checks.</p>'}</details>`).join('')}

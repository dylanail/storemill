...
function qualificationCard(ctx: Ctx, product: ReturnType<typeof listProducts>[number]): string {
  ...
  const result = qualifyCatalogProduct(product, {...notes,currency:ctx.store.currency})
  ...
  return `<div class="card" id="qualify"><div class="row" style="justify-content:space-between"><h2 style="margin:0">Product qualification</h2><span class="tag ${decision[result.decision]}">${{run:'run it',work:'work on it',skip:'skip it'}[result.decision]}</span></div><p class="muted" style="font-size:12px">${escapeHtml(result.summary)}</p><table class="data"><tbody>${result.checks.map((check) => `<tr><td><span class="tag ${tone[check.verdict]}">${check.verdict}</span> ${escapeHtml(check.label)}</td><td>${escapeHtml(check.detail)}<div class="muted" style="font-size:11px">${escapeHtml(check.rule)}</div></td></tr>`).join('')}</tbody></table><form method="post" action="/admin/products/${escapeHtml(product.id)}/qualify" style="margin-top:.7rem"><div class="row"><div class="field" style="flex:1"><label>Trend — Google Trends, US, five years</label><select name="trend">${['unknown', 'up', 'flat', 'declining', 'spike'].map((trend) => `<option value="${trend}" ${notes.trend === trend ? 'selected' : ''}>${({unknown:'Not checked',up:'Rising',flat:'Flat',declining:'Declining',spike:'Spiked and crashed'} as Record<string,string>)[trend]}</option>`).join('')}</select></div><div class="field"><label>Weight grams</label><input name="weightGrams" value="${notes.weightGrams ?? ''}"></div><div class="field"><label for="qualify-aov">Order value after bundles (${escapeHtml(ctx.store.currency)})</label><input id="qualify-aov" name="aovAmount" type="number" min="0" step="${1 / 10 ** minorDigits(ctx.store.currency)}" value="${notes.aovCents === undefined ? '' : (notes.aovCents / 10 ** minorDigits(ctx.store.currency)).toFixed(minorDigits(ctx.store.currency))}" placeholder="80"><small>Enter the amount in ${escapeHtml(ctx.store.currency)}, for example 80 for an order worth ${escapeHtml(format(80 * 10 ** minorDigits(ctx.store.currency), ctx.store.currency))}.</small></div></div><div class="row" style="flex-wrap:wrap">${flag('seasonal', 'Seasonal', Boolean(notes.seasonal))}${flag('tech', 'Tech/battery', Boolean(notes.tech))}${flag('patented', 'Patented', Boolean(notes.patented))}${flag('bigBrand', 'Big brand', Boolean(notes.bigBrand))}${flag('printOnDemand', 'Print on demand', Boolean(notes.printOnDemand))}</div><div class="field"><label>The way to stand out — an underserved buyer or a distinct mechanism</label><input name="standOut" value="${escapeHtml(notes.standOut ?? '')}" placeholder="Underserved avatar or unique mechanism"></div><p class="muted">If you cannot name a way to stand out, reconsider running this product. Price and landed cost come from the supplier card; the remaining inputs are your research. ${ctx.store.currency === 'USD' ? '' : 'The method uses US-dollar thresholds; compare non-USD amounts against those thresholds before making a decision.'}</p><button class="btn primary" type="submit">Save qualification</button></form></div>`
}
...
export function pagesPage(ctx: Ctx): string {
  ...
  <div class="grid3" style="margin-bottom:1.2rem">
    ...
    <form method="post" action="/admin/pages/clone" class="card"><h2>Copy from a link</h2>${copyOptions(true)}
      <div class="field"><label>URL</label><input name="url" type="url" required placeholder="https://"></div>
      <div data-copy-page-settings><div class="field"><label>Page type</label><select name="role">${roleOptions()}</select></div>
      ...
      <label class="row" style="font-size:12px;margin-bottom:.6rem"><input type="checkbox" name="keepScripts" value="true"> Keep scripts (pixels, chat widgets, the source's app)</label></div>
      <button class="btn primary" type="submit">Copy selected scope</button></form>
  ...
  ${pages.length ? pages.map((page) => `<tr><td><a href="/admin/pages/${escapeHtml(page.id)}/edit">${escapeHtml(page.title)}</a>${page.isHome ? ' <span class="tag ok">home</span>' : ''}${page.role === 'checkout' ? ` <span class="tag ${page.status === 'published' ? 'ok' : 'warn'}" title="The most recently updated published checkout page is the store's /checkout">checkout</span>` : ''}<div class="muted" style="font-size:11.5px">/pages/${escapeHtml(page.handle)}${page.sourceUrl ? ` · cloned from ${escapeHtml(page.sourceUrl.replace(/^https?:\/\//, '').slice(0, 40))}` : ''}</div></td>
    ...
    <td style="text-align:right"><div class="row" style="justify-content:flex-end"><a class="btn" href="/admin/pages/${escapeHtml(page.id)}/edit">Edit</a>
      ...
      <form method="post" action="/admin/pages/${escapeHtml(page.id)}/duplicate"><button class="btn">Duplicate page only</button></form>
...
export function storesPage(ctx: Ctx, stores: Store[]): string {
  ...
  <label class="field">Search stores and funnels<input type="search" id="asset-search" placeholder="Search by name"></label><div class="asset-tabs"><button class="on" type="button" data-filter="all">All ${stores.length}</button><button type="button" data-filter="store">Stores ${storeCount}</button><button type="button" data-filter="funnel">Funnels ${funnelCount}</button></div>
  <div class="asset-grid" id="asset-grid">${stores.map((store) => {
    ...
    return `<article class="asset-card" data-kind="${store.kind}" data-name="${escapeHtml(store.name.toLowerCase())}">
  ...
  <script>(function(){
    ...
    var selected='all',search=document.getElementById('asset-search');function filter(){cards.forEach(function(card){card.hidden=(selected!=='all'&&card.dataset.kind!==selected)||!card.dataset.name.includes(search.value.toLowerCase().trim())})}search.addEventListener('input',filter);buttons.forEach(function(button){button.addEventListener('click',function(){buttons.forEach(function(item){item.classList.remove('on')});button.classList.add('on');selected=button.dataset.filter;filter()})});

...
export function mediaPage(ctx: Ctx): string {
  ...
  return `${mediaRebrandStyle}${flash(ctx)}<div class="head"><div><div class="eyebrow">${ctx.store.kind} asset</div><h1 class="serif">Media</h1><p class="muted">Keep brand logos separate from product images and videos. Use logos as references when regenerating media in the page editor.</p></div><span class="tag">${logos.length} logos · ${media.length} other media</span></div>

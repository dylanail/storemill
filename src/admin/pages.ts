...
type Ctx = { db: Db; store: Store; userName: string; userEmail?: string; storeUrl: string; flash?: string }
...
export function pagesPage(ctx: Ctx): string {
  ...
  <div class="grid3" style="margin-bottom:1.2rem">
    ...
    <form method="post" action="/admin/pages/clone" class="card"><h2>Clone a reference page or funnel</h2><div class="field"><label for="clone-scope">What to clone</label><select id="clone-scope" name="scope"><option value="page">This page only</option><option value="funnel">Whole funnel — all linked steps</option></select></div><details><summary>Additional funnel steps</summary><label class="field">Direct URLs for unlinked steps<textarea name="additionalUrls" rows="3" placeholder="https://example.com/upsell"></textarea></label><p class="muted">Whole-funnel cloning creates a separate funnel asset with its products, media and steps. Payment-only steps require direct URLs.</p></details>
...
export function mediaPage(ctx: Ctx): string {
  ...
  const storeQuery = `storeId=${encodeURIComponent(ctx.store.id)}`
  return `${mediaRebrandStyle}${flash(ctx)}<div class="head"><div><div class="eyebrow">${ctx.store.kind} asset</div><h1 class="serif">Media</h1><p class="muted" style="margin:.25rem 0 0">Images and videos used by ${escapeHtml(ctx.store.name)}. Choose Rebrand to redo an existing asset with your desired logo or brand.</p></div><span class="tag">${assets.length} media · ${local} locally owned</span></div>
  <form method="post" action="/admin/media/upload?${storeQuery}" enctype="multipart/form-data" class="card media-upload"><div><h2>Add an image or video</h2><p class="muted">Images up to 12MB; MP4, WebM or MOV videos up to 100MB.</p></div><input type="file" name="image" aria-label="Upload image or video" accept="image/*,video/mp4,video/webm,video/quicktime" required><button class="btn primary" type="submit">Upload</button></form>
  ${assets.length ? `<div class="media-grid">${assets.map((asset) => `<figure class="media-card">${asset.kind === 'video' ? `<video src="${escapeHtml(asset.url)}" controls playsinline preload="metadata" aria-label="${escapeHtml(asset.label)}"></video>` : asset.kind === 'embed' ? `<a href="${escapeHtml(asset.url)}" target="_blank" rel="noopener" class="card">Embedded video · open player</a>` : `<a href="${escapeHtml(asset.url)}" target="_blank" rel="noopener"><img src="${escapeHtml(asset.url)}" alt="${escapeHtml(asset.label)}" loading="lazy"></a>`}<figcaption><div><strong>${escapeHtml(asset.label)}</strong><span>${escapeHtml(asset.source)} · ${asset.kind}${size(asset.bytes) ? ` · ${size(asset.bytes)}` : ''}</span></div><div class="media-actions">${asset.kind !== 'embed' ? `<a class="btn primary" href="/admin/media/rebrand?${storeQuery}&source=${encodeURIComponent(asset.url)}">Rebrand</a>` : '<span class="muted">Upload the original video to rebrand</span>'}<button class="btn" type="button" data-copy="${escapeHtml(asset.url)}">Copy URL</button></div></figcaption></figure>`).join('')}</div>` : '<div class="card cro-empty"><div class="cro-orb">'+uiIcon('image', 24)+'</div><h2>No media yet</h2><p class="muted">Upload a photo or video, clone a reference, or generate imagery from the Assistant.</p></div>'}

...
(() => {
  ...
  function sync(){
    ...
    dialog.querySelectorAll('[data-brand]').forEach(section=>section.hidden=ai&&!['rebrand','custom'].includes(value('intent')));q('[data-ai]').hidden=!ai;q('[data-overlay]').hidden=ai;q('[name=direction]').disabled=!ai;
  ...
  function picker(config){pick=config;q('[data-edit-panel]').hidden=true;q('[data-picker-panel]').hidden=false;dialog.scrollTop=0;q('[data-picker-title]').textContent=config.title;q('[name=assetCategory]').value=config.category||'';q('[name=assetSearch]').value='';q('[name=assetUpload]').accept=config.kind==='video'?'video/mp4,video/webm,video/quicktime':'image/*';renderPicker();q('[name=assetSearch]').focus();}
  ...
  async function generate(){
    ...
    if(spec.method==='ai'&&['custom','text'].includes(spec.intent)&&!spec.direction.trim()){say('Describe what you want to change.',true);q('[name=direction]').focus();return;}
    if(spec.method==='ai'&&!['rebrand','custom'].includes(spec.intent)){spec.logo='';spec.oldBrand='';}
  ...
  async function open(input){
    ...
    dialog.innerHTML='<div class="em-heading"><h2>Regenerate or edit '+esc(input.kind)+'</h2><button class="btn" type="button" data-close aria-label="Close media editor">Close</button></div><div class="em-body"><p class="em-status" role="status" aria-live="polite">Loading your media library…</p><div data-content></div></div>';
    ...
    q('[data-content]').innerHTML=`<div data-edit-panel><div class="em-tabs" role="tablist"><button class="btn" type="button" data-tab="edit" role="tab" aria-selected="true">Regenerate</button><button class="btn" type="button" data-tab="replace" role="tab" aria-selected="false">Replace / upload</button></div><div class="em-grid"><div>${input.source&&!input.embed?preview(input.source,input.kind,'Current '+input.kind):'<p>Choose or upload the original '+esc(input.kind)+' file to begin.</p>'}<div data-result></div><p class="em-note">${input.kind==='video'?'AI video edits support complete 2–30 second clips up to 1080p. Exact logo overlays support clips up to 5 minutes and 100MB.':'Images up to 12MB. AI creates a new version; review lettering and product details before using it. Animated images become a still image.'}</p>${input.note?'<p class="em-note">'+esc(input.note)+'</p>':''}<label class="em-scope">Apply replacement to<select name="scope"><option value="selected">Selected placement only</option><option value="page">All matching placements on this page</option></select></label><button class="btn primary" type="button" data-use-result hidden>Use this version</button></div><div><div data-regenerate>
      <div data-brand><label>Desired brand name<input name="brandName" maxlength="100" value="${esc(defaults.brandName)}"></label>
      </div><label>Editing method<select name="method"><option value="ai">AI edit</option><option value="overlay">Exact logo overlay</option></select></label>
      <div data-brand><label>Desired logo<img class="em-logo" alt="Desired logo" hidden></label><div class="em-actions"><button class="btn" type="button" data-logo>Choose logo asset</button><button class="btn" type="button" data-logo-upload>Upload logo</button><button class="btn" type="button" data-logo-clear>No logo</button></div><input type="file" name="logoUpload" accept="image/png,image/jpeg,image/webp,image/svg+xml" hidden></div>
      <div data-ai><label>Edit goal<select name="intent"><option value="variation">Similar variation · keep pose and setting</option><option value="rebrand">Replace branding</option><option value="text">Change words</option><option value="custom">Custom instructions</option><option value="cleanup">Remove objects or unwanted text</option><option value="background">Change background</option><option value="enhance">Improve lighting and clarity</option><option value="restyle">Restyle the scene</option></select></label><label>Image editor<select name="provider">${options.available.images.map(provider=>'<option value="'+provider.id+'" '+(!provider.available?'disabled':'')+'>'+esc(provider.name)+(!provider.available?' — not connected':'')+'</option>').join('')}</select></label></div>
      <label>Edit suggestions<textarea name="direction" rows="5" maxlength="6000" placeholder="For example: keep a similar pose and setting, with slightly different lighting. Or specify new wording, branding, background or style."></textarea></label>
      <div data-ai-extra><label>Keep unchanged<textarea name="preserve" rows="2" maxlength="1500" placeholder="For example: similar pose and setting, product shape, camera angle, and existing label text."></textarea></label><div class="em-references"></div><div class="em-actions"><button class="btn" type="button" data-reference>Choose reference image</button><button class="btn" type="button" data-reference-upload>Upload reference images</button></div><input type="file" name="referenceUpload" accept="image/*" multiple hidden><p class="em-note">Up to three reference images, in addition to your logo. Uploads are also saved in Media.</p><details><summary>More options</summary><label data-brand>Brand to replace<input name="oldBrand" maxlength="100"></label><label data-shape>Output shape<select name="shape"><option value="original">Keep original dimensions</option><option value="square">Square · 1024 × 1024</option><option value="landscape">Landscape · 1536 × 1024</option><option value="portrait">Portrait · 1024 × 1536</option></select></label></details></div>
    ...
    const syncAll=()=>{sync();q('[data-ai-extra]').hidden=value('method')!=='ai';};q('[name=method]').onchange=syncAll;q('[name=intent]').onchange=syncAll;
    q('[data-logo]').onclick=()=>picker({title:'Choose your desired logo',kind:'image',onChoose:asset=>{logo=asset.url;showLogo();}});
